...
for mlp_depth in [2, 3, 4, 6, 8]:
    filename = f"spiral_20_NN_train_temb{temb_dim}_depth{mlp_depth}_width{mlp_width}_batch2048_ep5000.pkl"
    try:
...
temb_dim = 32
plt.figure()
for mlp_width in [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]:
    filename = f"spiral_20_NN_train_temb{temb_dim}_depth{mlp_depth}_width{mlp_width}_batch2048_ep5000.pkl"
    try:
        data = pkl.load(open(join(expdir, filename), "rb"))
        meta, loss_traj = data
